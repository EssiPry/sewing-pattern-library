...
def get_patterns(name, company, fabric):
    sql = """SELECT P.name, P.company, P.fabric
             FROM patterns P
...
def get_garments(pattern_name):
    sql = """SELECT G.garment
             FROM garments G, garments_in_pattern A, patterns P
             WHERE A.garment_id = G.id AND A.pattern_id = P.id AND P.name=:pattern_name"""
